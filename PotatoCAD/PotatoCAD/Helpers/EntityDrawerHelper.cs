...
using static PotatoCAD.ReadDxfForm;
...
namespace PotatoCAD.Helpers
{
    public static class EntityDrawerHelper
    {
        ...
        public static async Task Dimension(ViewerHandle viewer, List<DimensionItem> batch)
        {
            ...
            int[] cR = batch.Select(d => (int)AcadColorToRgb(d.DimColor).r).ToArray();
            int[] cG = batch.Select(d => (int)AcadColorToRgb(d.DimColor).g).ToArray();
            int[] cB = batch.Select(d => (int)AcadColorToRgb(d.DimColor).b).ToArray();
