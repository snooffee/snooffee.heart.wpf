...
gp_Pnt MouseHelper::Get3DPntFromScreen(Handle(V3d_View) view, int x, int y)
{
    Standard_Real X, Y, Z;
    view->Convert(x, y, X, Y, Z);
    return gp_Pnt(X, Y, 0);
}
Handle(AIS_Shape) MouseHelper::DrawLine(NativeViewerHandle* native, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
    return LineDrawer::DrawLine(view, viewerHandlePtr, native->dragStartX, native->dragStartY, native->dragEndX, native->dragEndY, h, w);
}
Handle(AIS_Shape) MouseHelper::DrawCircle(NativeViewerHandle* native, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
...
Handle(AIS_Shape) MouseHelper::DrawRectangle(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
    return RectangleDrawer::DrawRectangle(context, native, view);
}
Handle(AIS_Shape) MouseHelper::DrawEllipse(NativeViewerHandle* native, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
...
void MouseHelper::ResetDragState(NativeViewerHandle* native)
{
...
void MouseHelper::ClearRubberBand(NativeViewerHandle* native)
{
    if (!native) return; // 🛡️ Safety check
...
void MouseHelper::HandleMoveModeMouseDown(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y)
{
...
void MouseHelper::HandleRotateModeMouseDown(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y)
{
    context->MoveTo(x, y, view, Standard_True);
    if (!context->HasDetected()) return;
...
void MouseHelper::HandleDefaultMouseDown(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y, bool multipleselect, std::vector<Handle(AIS_InteractiveObject)>& lastHilightedObjects)
{
...
void MouseHelper::HandleMateModeMouseDown(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y)
{
    context->Deactivate();
    ...
    else
    {
        ...
        MateHelper::HandleFaceAlignment(context, view, native->firstMateShape, face1, detected, face2);
...
void MouseHelper::HandleLineMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w, int x, int y)
{
...
void MouseHelper::HandleCircleMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
...
void MouseHelper::HandleEllipseMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
    ...
    ViewHelper::ClearCreateEntity(native); // Reset state if needed (based on your existing methods)
}
void MouseHelper::HandleRectangleMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, IntPtr viewerHandlePtr, int h, int w)
{
...
void MouseHelper::HandleTrimMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context)
{
}
void MouseHelper::HandleRadiusMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context)
{
}
void MouseHelper::HandleExtrudingMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view)
{
...
void MouseHelper::HandleBooleanMode(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y)
{
    native->isDragging = false;
    context->Activate(TopAbs_SHAPE);
    context->MoveTo(x, y, view, Standard_True);
...
void MouseHelper::HandleObjectDetection(Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int x, int y)
{
    if (context->HasDetected())
    {
        Handle(AIS_InteractiveObject) detectedIO = context->DetectedInteractive();
...
void MouseHelper::DrawLineOverlay(NativeViewerHandle* native, int height)
{
...
void MouseHelper::DrawCircleOverlay(NativeViewerHandle* native, int height)
{
...
void MouseHelper::DrawEllipseOverlay(NativeViewerHandle* native, int height)
{
...
void MouseHelper::DrawRectangleOverlay(NativeViewerHandle* native, int height)
{
...
void MouseHelper::DrawDimensionOverlay(NativeViewerHandle* native, Handle(V3d_View) view, const gp_Pnt& p1, const gp_Pnt& p2, const gp_Pnt& cursor, bool isFinal)
{
...
void MouseHelper::DrawSelectionRectangle(NativeViewerHandle* native)
{
...
void MouseHelper::HighlightHoveredShape(NativeViewerHandle* native, int x, int y, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view)
{
    context->Deactivate();
    context->Activate(TopAbs_SHAPE);
    context->MoveTo(x, y, view, Standard_True);
...
void MouseHelper::HandleMove(NativeViewerHandle* native, int x, int y, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view)
{
    Standard_Real X, Y, Z;
    view->Convert(x, y, X, Y, Z);
...
void MouseHelper::HandleRotate(NativeViewerHandle* native, int x, int y, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view)
{
    ...
    // ✅ Combine with existing local transformation
    gp_Trsf currentTrsf = aisShape->LocalTransformation();
    gp_Trsf newTrsf = currentTrsf.Multiplied(combinedTrsf);

    // Apply new transformation to AIS_Shape (keeps geometry intact)
...
void MouseHelper::HandleExtrude(NativeViewerHandle* native, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view, int y)
{
...
void MouseHelper::UpdateHoverDetection(NativeViewerHandle* native, int x, int y, Handle(AIS_InteractiveContext) context, Handle(V3d_View) view)
{
    context->Deactivate();
    context->Activate(TopAbs_SHAPE);
    context->MoveTo(x, y, view, Standard_True);
