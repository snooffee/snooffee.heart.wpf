...
void ShapeDrawer::UpdateSelection(System::IntPtr viewerHandlePtr, int x, int y, bool isClick)
{
    ...
    if (context->HasDetected())
    {
        ...
        std::cout << "[PotaOCC] UpdateSelection: HasDetected = true" << std::endl;
        std::cout << "[PotaOCC]   DetectedInteractive is null? " << (detIo.IsNull() ? 1 : 0) << std::endl;
        if (!detShape.IsNull())
        {
            std::cout << "[PotaOCC]   DetectedShape type = " << (int)detShape.ShapeType() << std::endl;
        }
        else
        {
            std::cout << "[PotaOCC]   DetectedShape is null" << std::endl;
        }
    }
    else
    {
        std::cout << "[PotaOCC] UpdateSelection: HasDetected = false" << std::endl;
    }
...
void ShapeDrawer::AlignViewToSelectedFace(System::IntPtr viewerHandlePtr)
{
    ...
    {
        std::cout << "[PotaOCC] AlignViewToSelectedFace: using SelectedShape()" << std::endl;
        ...
        if (!selShape.IsNull() && selShape.ShapeType() == TopAbs_FACE)
        {
            ...
            if (nvec.Magnitude() > Precision::Confusion())
            {
                ...
                native->view->FitAll();
                native->view->Redraw();
                std::cout << "[PotaOCC] AlignViewToSelectedFace: aligned to selected face." << std::endl;
                return;
            }
        }
        std::cout << "[PotaOCC] AlignViewToSelectedFace: selected item not a face or normal compute failed." << std::endl;
    }
    else
    {
        std::cout << "[PotaOCC] AlignViewToSelectedFace: no selected items." << std::endl;
    }

    // Fallback: use the currently detected shape under cursor (if any)
    if (native->context->HasDetected())
    {
        std::cout << "[PotaOCC] AlignViewToSelectedFace: falling back to DetectedShape()" << std::endl;
        ...
        if (!detShape.IsNull() && detShape.ShapeType() == TopAbs_FACE)
        {
            ...
            if (nvec.Magnitude() > Precision::Confusion())
            {
                ...
                native->view->FitAll();
                native->view->Redraw();
                std::cout << "[PotaOCC] AlignViewToSelectedFace: aligned to detected face." << std::endl;
                return;
            }
        }
        std::cout << "[PotaOCC] AlignViewToSelectedFace: detected item not a face or normal compute failed." << std::endl;
    }
    else
    {
        std::cout << "[PotaOCC] AlignViewToSelectedFace: no detected shape to fallback on." << std::endl;
    }
}
...
void ShapeDrawer::ConvertLineToCenterLine(
    const Handle(AIS_InteractiveContext)& context,
    const TopoDS_Edge& edge)
{
    if (edge.IsNull()) {
        std::cout << "⚠️ Invalid edge for center line conversion." << std::endl;
        return;
    }

    // Get geometry info (for debugging or logging)
    Standard_Real f, l;
    Handle(Geom_Curve) curve = BRep_Tool::Curve(edge, f, l);
    if (curve.IsNull()) {
        std::cout << "⚠️ Could not retrieve curve geometry from edge." << std::endl;
        return;
    }

    // Display as dashed gray line
    Handle(AIS_Shape) aisCenterLine = new AIS_Shape(edge);
    Handle(Prs3d_Drawer) drawer = aisCenterLine->Attributes();
    Handle(Prs3d_LineAspect) lineAspect = new Prs3d_LineAspect(
        Quantity_Color(Quantity_NOC_GRAY), Aspect_TOL_DASH, 2.0);
    drawer->SetLineAspect(lineAspect);

    // Optionally mark as auxiliary/construction object
    drawer->SetShadingAspect(new Prs3d_ShadingAspect());
    drawer->SetWireAspect(lineAspect);
    aisCenterLine->SetAttributes(drawer);

    // Optional: name or color coding
    context->Erase(aisCenterLine, Standard_False);
    context->Display(aisCenterLine, Standard_True);

    std::cout << "✅ Converted selected line to center line." << std::endl;
}

Handle(AIS_Shape) ShapeDrawer::DisplayCenterLine(
    const Handle(AIS_InteractiveContext)& context,
    const TopoDS_Edge& edge)
{
    if (edge.IsNull()) {
        std::cout << "⚠️ Invalid edge for center line display." << std::endl;
        return nullptr;
    }
    ...
    Handle(Prs3d_Drawer) drawer = aisCenterLine->Attributes();
    Handle(Prs3d_LineAspect) dashedLineAspect =
        new Prs3d_LineAspect(Quantity_Color(Quantity_NOC_GRAY), Aspect_TOL_DASH, 2.0);
    drawer->SetLineAspect(dashedLineAspect);
    drawer->SetWireAspect(dashedLineAspect);
    drawer->SetTransparency(0.4);
    aisCenterLine->SetAttributes(drawer);

    context->Display(aisCenterLine, Standard_True);
    std::cout << "✅ Center line displayed (dashed gray)." << std::endl;

    return aisCenterLine;
}

Handle(AIS_Shape) ShapeDrawer::CreateSafeAISShape(const gp_Pnt& start, const gp_Pnt& end)
{
    gp_Vec vec(start, end);
    if (vec.Magnitude() < 1e-6) return nullptr; // too short, skip

    BRepBuilderAPI_MakeEdge edgeMaker(start, end);
    if (!edgeMaker.IsDone()) return nullptr;

    try {
        Handle(AIS_Shape) aisShape = new AIS_Shape(edgeMaker.Edge());
        return aisShape;
    }
    catch (Standard_Failure& e) {
        std::cerr << "[CreateSafeAISShape] AIS_Shape creation failed: " << e.GetMessageString() << std::endl;
        return nullptr;
    }
}