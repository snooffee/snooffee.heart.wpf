    ...
    double sceneScale)
{
    ...
    PotaOCC::NativeViewerHandle* native =
        reinterpret_cast<PotaOCC::NativeViewerHandle*>(viewerHandlePtr.ToPointer());
...
Handle(AIS_Shape) TextDrawer::DrawSingleText(
    System::IntPtr viewerHandlePtr,
    ...
    int r, int g, int b)
{
    if (viewerHandlePtr == System::IntPtr::Zero) return nullptr;
    NativeViewerHandle* native = reinterpret_cast<NativeViewerHandle*>(viewerHandlePtr.ToPointer());
    if (!native || native->context.IsNull() || native->view.IsNull()) return nullptr;

    Handle(AIS_InteractiveContext) context = native->context;

    array<double>^ xs = gcnew array<double>(1) { x };
    array<double>^ ys = gcnew array<double>(1) { y };
    array<double>^ zs = gcnew array<double>(1) { z };
    array<System::String^>^ texts = gcnew array<System::String^>(1) { text };
    array<double>^ heights = gcnew array<double>(1) { height };
    array<double>^ rotations = gcnew array<double>(1) { rotationDeg };
    array<double>^ widthFactors = gcnew array<double>(1) { widthFactor };
    array<int>^ rr = gcnew array<int>(1) { r };
    array<int>^ gg = gcnew array<int>(1) { g };
    array<int>^ bb = gcnew array<int>(1) { b };
    array<double>^ transparency = gcnew array<double>(1) { 0.0 };

    // This version does NOT store in native->ais2DShapes
    TextDrawer::DrawTextBatch(viewerHandlePtr, xs, ys, zs, texts, heights, rotations, widthFactors, rr, gg, bb, transparency, 1.0);

    // Return the AIS_Shape of the temporary label
    if (!native->ais2DShapes.empty())
        return native->ais2DShapes.back(); // get last shape
    return nullptr;
}