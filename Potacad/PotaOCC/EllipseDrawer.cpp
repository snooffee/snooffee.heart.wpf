...
TopoDS_Edge EllipseDrawer::DrawEllipse(Handle(V3d_View) view, IntPtr viewerHandlePtr, double dragStartX, double dragStartY, double dragEndX, double dragEndY, int h, int w)
{
    Handle(AIS_OverlayEllipse) persistentEllipse = new AIS_OverlayEllipse();
    ShapeDrawer::SetupOverlay(persistentEllipse, Quantity_NOC_GREEN, 2.0);  // Use a different color for ellipse
    ...
    // Calculate horizontal and vertical radii (X and Y distances)
    double radiusX = std::abs(center.X() - edgePoint.X());  // Horizontal radius
    double radiusY = std::abs(center.Y() - edgePoint.Y());  // Vertical radius

    // Create an ellipse using the calculated radii
    gp_Elips ellipse(gp_Ax2(center, gp_Dir(0, 0, 1)), radiusX, radiusY);

    return ShapeDrawer::CreateEllipseSafe(ellipse);
}